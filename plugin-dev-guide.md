...
| 背景贴片 | `<rect width="64" height="64" rx="14" fill="url(#pg)"/>` + `<defs>` 里一条**同色系** `linearGradient`，满幅、不内缩 |
| 图形主色 | 白 `#fff`，落在安全区 `12..52` 内 |
...
| 文件 | 用途 |
|---|---|
| `icon.svg` | **深色主题**图标；也是所有消费方的默认回退 |
| `icon.light.svg` | **浅色主题**图标 |

解析顺序：浅色主题先找 `icon.light.svg`，找不到就用 `icon.svg`；深色主题始终用 `icon.svg`。
所以**只带一个图标的老插件不用改也能正常工作**，但两档齐备观感最好。

两档之间的差别**只在色值与渐变端**，形状、线宽、构图必须完全一致：

- 深色档用**亮档**锚点（相对亮度 ≈ 0.235）—— 同一个色值直接放到深色界面上会发闷、沉下去
- 浅色档用**深档**锚点（相对亮度 ≈ 0.170）—— 亮档放到浅色界面上会发飘、发白
- 渐变端由锚点色推出，**两档都是「比锚点更深」**（见下节）

### 语义色板

插件图标颜色**只能**取自下表（6 个语义色，按插件功能归类，不按个人喜好）。每色两档由 OKLCH
求解，硬约束是「白色图形对贴片**两端**都 ≥ 3:1」且「贴片两端对各自主题背景都 ≥ 3:1」：

| 语义 | 用途 | 深色档（锚点 → 渐变端） | 浅色档（锚点 → 渐变端） |
|---|---|---|---|
| `blue` | 通用工具 / 转换 / 文档 | `#3381ff` → `#5957e0` | `#1d66ff` → `#5134dd` |
| `violet` | 开发与代码 | `#916aff` → `#9740cd` | `#7a50ff` → `#8417cd` |
| `teal` | 网络与连通 | `#1c958a` → `#0f777f` | `#148077` → `#03636b` |
| `amber` | 安全与探测 | `#d46615` → `#9b6215` | `#ba560e` → `#855108` |
| `green` | 数据与系统 | `#24993e` → `#1d7959` | `#10852b` → `#036647` |
| `pink` | 媒体与图像 | `#ea4683` → `#d1243e` | `#d32f6c` → `#b6102a` |
...
节点、连线、外框这类**结构性元素**一律保持白色，硬塞彩色只会显得杂。
