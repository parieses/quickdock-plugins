...
| 背景贴片 | `<rect width="64" height="64" rx="14" fill="<语义色>"/>`，满幅、不内缩 |
| 图形 | 一律**白色**（`#fff`），落在安全区 `12..52` 内 |
...
两档之间的差别**只在品牌色明度档**，形状、线宽、构图必须完全一致：

- 深色档用**亮档**（相对亮度 ≈ 0.235）—— 同一个色值直接放到深色界面上会发闷、沉下去
- 浅色档用**深档**（相对亮度 ≈ 0.170）—— 亮档放到浅色界面上会发飘、发白

### 语义色板

插件图标颜色**只能**取自下表（6 个语义色，按插件功能归类，不按个人喜好）。每色两档由 OKLCH
求解，硬约束是「白色图形对贴片底色 ≥ 3:1」且「贴片对各自主题背景 ≥ 3:1」：

| 语义 | 用途 | 深色档 | 浅色档 |
|---|---|---|---|
| `blue` | 通用工具 / 转换 / 文档 | `#3381ff` | `#1d66ff` |
| `violet` | 开发与代码 | `#916aff` | `#7a50ff` |
| `teal` | 网络与连通 | `#1c958a` | `#148077` |
| `amber` | 安全与探测 | `#d46615` | `#ba560e` |
| `green` | 数据与系统 | `#24993e` | `#10852b` |
| `pink` | 媒体与图像 | `#ea4683` | `#d32f6c` |

新增插件时在 `gen_icons.py` 的 `GROUP` 字典里登记插件目录 → 语义色，再跑一次 `--gen`。

### 从旧图标迁移

老插件的图标写法五花八门（裸线稿、暗灰贴片、内缩色卡、渐变卡底、圆形描边…）。
`--gen` 会就地把它规格化，**不改美术，只做四件事**：

1. **剥卡底** —— 删掉旧图标自带的整块底色矩形，统一换成规范贴片。
   判据是「实心填充 + 两轴 ≥ 70% viewBox + 基本居中」，`fill="none"` 的画框天然不会被误伤。
2. **图形转白** —— 所有颜色值归一成 `#fff`，保留 `fill="none"` 与 `url(#..)` 引用。
3. **归一线宽** —— 让最粗的一笔在 64 网格上恰为 `3`，细部按同一系数缩放。
4. **缩放进安全区** —— 图形包围盒贴合 `12..52`，居中。

两个必须知道的坑（都会让图标变成一块白方，图形彻底看不见）：
...
所以 `--check-render` 会把「白像素 > 45%」和「贴片色 < 25%」直接判为错误。

**已知例外**：`<text>` 里的 emoji 不受 `fill` 控制（彩色字形由字体决定），
如 `emoji-search`。这类图标保留原样，属于可接受的例外。
