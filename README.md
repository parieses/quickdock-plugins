...
1. **从文件安装**：点「从文件安装」选择 **Windows 安装包 zip**（Release 资产）；
2. **拖拽安装**：直接把 zip 拖进插件管理页。

安装即生效、无需重启；升级覆盖安装自动备份旧版本，失败自动回滚。
zip 下载见 [Releases](https://github.com/parieses/quickdock-plugins/releases) 或[插件中心主页](https://parieses.github.io/quickdock-plugins/)。
...
# 生成插件中心主页（扫描各插件 plugin.json，输出到 site/index.html）
python gen_site.py
```
...
| `plugin:result` | 执行结果 `{id, result \| error}` |
...
1. 改 `plugin.json` 的 `version`
2. `git add . && git commit`
3. 推送后 CI 自动出 windows zip 到 `dist/`（或本地 `python build.py <插件目录>`）
4. 创建 GitHub Release（tag 如 `disk-analyzer-v0.1.1`），把该插件 windows 的 zip（`dist/` 下）上传为资产 —— 主页/下载链接依赖 `releases/latest/download/<id>-windows.zip`

> 注：主程序 QuickDock 目前仅发布 Windows 版本，故插件默认只打包 windows。需要其他平台时在 build.py 命令后加 `--platform all`（或 `darwin`/`linux`）。
